...
func TestCanItBeRecycled_invalidBarcode(t *testing.T) {
	...
	for _, bc := range cases {
		_, err := svc.CanItBeRecycled(context.Background(), &pb.CanItBeRecycledRequest{Barcode: bc})
...
func TestCanItBeRecycled_notFound(t *testing.T) {
	resolver := &stubResolver{err: providers.ErrNotFound}
	svc := newTestService(t, resolver, &stubClassifier{})

	_, err := svc.CanItBeRecycled(context.Background(), &pb.CanItBeRecycledRequest{Barcode: "12345678"})
	if status.Code(err) != codes.NotFound {
		t.Errorf("expected NotFound, got %v", err)
	}
}

func TestCanItBeRecycled_upstreamError(t *testing.T) {
...
func TestCanItBeRecycled_cacheHit(t *testing.T) {
	...
	req := &pb.CanItBeRecycledRequest{Barcode: "12345678"}
	_, _ = svc.CanItBeRecycled(context.Background(), req)
	_, _ = svc.CanItBeRecycled(context.Background(), req)
